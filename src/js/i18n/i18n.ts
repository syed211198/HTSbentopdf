...
// Supported languages
export const supportedLanguages = ['en', 'de', 'zh', 'vi', 'it', 'id'] as const;
...
export const getLanguageFromUrl = (): SupportedLanguage => {
  ...
  const langMatch = path.match(/^\/(en|de|zh|vi|it|id)(?:\/|$)/);
  if (
    langMatch &&
...
export const changeLanguage = (lang: SupportedLanguage): void => {
  ...
  let newPath: string;
  if (currentPath.match(/^\/(en|de|zh|vi|it|id)\//)) {
    newPath = currentPath.replace(/^\/(en|de|zh|vi|it|id)\//, `/${lang}/`);
  } else if (currentPath.match(/^\/(en|de|zh|vi|it|id)$/)) {
    newPath = `/${lang}`;
  } else {
...
export const rewriteLinks = (): void => {
  ...
  links.forEach((link) => {
    ...
    if (href.match(/^\/(en|de|zh|vi|it|id)\//)) {
      return;
    }
    let newHref: string;
    if (href.startsWith('/')) {
